module.exports = function(grunt) {
    ...
    var commands = [];
    var subdirectories = {};
    commands.push('rmdir ' + output_directory);
    commands.push('mkdir ' + output_directory);
    commands.push('mkdir ' + output_directory + '\\temp');
    for (var i = 0; i < desired_sizes.length; i++) {
      ...
      // using imagemagick in windows
      var commandStr = 'convert ' + directory + '\\' + src_img +
          ' -resize ' + width + 'x' + height +
          ' -size ' + width + 'x' + height +
          ' xc:white +swap -gravity center -composite' +
          ' ' + directory + '\\output\\' + desired_size.replace('/', '\\') + '.png'
      console.log("make command=" + commandStr);
      commands.push(commandStr);
    }
    commands.push('rmdir ' + output_directory + '\\temp');
